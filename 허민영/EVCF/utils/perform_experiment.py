...
def experiment_vae(args, train_loader, val_loader, test_loader, model, optimizer, dir, log_dir, model_name='vae'):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_loss_epoch, val_re_epoch, val_kl_epoch, val_ndcg_epoch, pred_val_copy = evaluate(args, model, train_loader, val_loader, epoch, dir, mode='validation')
        time_end = time.time()
        # print("~~~~", pred_val_copy, pred_val_copy.shape)
        ...
        if val_ndcg_epoch > best_ndcg:
            ...
            # best_model = model
            print('->model saved<-')
            torch.save(model, dir + args.save) #  + '.model.pt'
        else:
            e += 1
